   ...
   public class MaskUtil
   {
      ...
      mx_internal static function applyMask(param1:DisplayObject, param2:DisplayObjectContainer) : void
      {
         ...
         if(_loc3_)
         {
            if(param2)
            {
               UIComponent(param2).addingChild(_loc3_);
               UIComponent(param2).childAdded(_loc3_);
            }
      ...
      {
         var _loc6_:LuminosityMaskShader = null;
         var _loc7_:ShaderFilter = null;
         if(!param1)
         {
            return;
         }
         ...
         else if(param2 == MaskType.LUMINOSITY)
         {
            param1.cacheAsBitmap = true;
            param5.cacheAsBitmap = true;
            _loc6_ = new LuminosityMaskShader();
            _loc6_.mode = calculateLuminositySettings(param3,param4);
