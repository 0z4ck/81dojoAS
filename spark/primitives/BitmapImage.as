   ...
   public class BitmapImage extends GraphicElement
   {
      ...
      override protected function measure() : void
      {
         ...
         else
         {
            ...
            _loc4_ = !!_loc3_?Number(measuredWidth):Number(0);
            _loc5_ = !!_loc3_?Number(measuredHeight):Number(0);
            measuredWidth = !isNaN(this._preliminaryWidth) && _loc4_ == 0?Number(this._preliminaryWidth):Number(_loc4_);
            measuredHeight = !isNaN(this._preliminaryHeight) && _loc5_ == 0?Number(this._preliminaryHeight):Number(_loc5_);
            return;
         }
      ...
      override protected function updateDisplayList(param1:Number, param2:Number) : void
      {
         ...
         if(!this._bitmapData || !drawnDisplayObject || !(drawnDisplayObject is Sprite))
         {
            if(this.loadedContent)
            {
               ...
               {
                  _loc16_ = this._fillMode == BitmapFillMode.CLIP || _loc5_?Number(this.imageWidth):Number(_loc4_);
                  _loc17_ = this._fillMode == BitmapFillMode.CLIP || _loc5_?Number(this.imageHeight):Number(_loc3_);
                  if(param2 > _loc17_)
                  {
         ...
         var _loc8_:Graphics = Sprite(drawnDisplayObject).graphics;
         _loc8_.lineStyle();
            ...
            switch(this._fillMode)
            {
               ...
               case BitmapFillMode.SCALE:
                  if(_loc5_)
                  {
                     ...
                     if(_loc18_ < _loc19_)
                     {
                        _loc10_ = _loc11_ = _loc3_ / this._bitmapData.height;
                     }
                     else
                     {
                        _loc10_ = _loc11_ = _loc4_ / this._bitmapData.width;
                     }
                  }
                  else
                  {
         ...
         if(this._fillMode != BitmapFillMode.SCALE || isNaN(this._scaleGridTop) || isNaN(this._scaleGridBottom) || isNaN(this._scaleGridLeft) || isNaN(this._scaleGridRight))
         {
            ...
            if(this.maintainAspectRatio || this._fillMode == BitmapFillMode.CLIP || _loc5_)
            {
               if(param2 > _loc24_)
               {
                  _loc13_ = _loc13_ + Math.floor((param2 - _loc24_) * this.getVerticalAlignValue());
               }
               if(param1 > _loc25_)
               {
                  _loc12_ = _loc12_ + Math.floor((param1 - _loc25_) * this.getHorizontalAlignValue());
               }
            }
            _loc26_ = _loc12_;
            _loc27_ = _loc13_;
            if(_loc5_)
            {
               if(_loc25_ > param1)
               {
                  _loc26_ = _loc26_ + (param1 - _loc25_) * this.getHorizontalAlignValue();
               }
               else if(_loc24_ > param2)
               {
                  _loc27_ = _loc27_ + (param2 - _loc24_) * this.getVerticalAlignValue();
               }
            }
            matrix.identity();
         ...
         else
         {
            ...
            while(_loc30_ < 3)
            {
               ...
               while(_loc33_ < 3)
               {
                  ...
                  matrix.translate(_loc29_.x - _loc28_.x * matrix.a,_loc29_.y - _loc28_.y * matrix.d);
      ...
      mx_internal function applySource() : void
      {
         ...
         {
            _loc1_ = this.getActualValue(_loc1_ as MultiDPIBitmapSource);
         }
         ...
         {
            this.loadExternal(_loc1_);
         }
         else if(_loc1_ is ByteArray)
         {
            this.loadFromBytes(_loc1_ as ByteArray);
         }
      ...
      {
         if(this.loadedContent && this.loadedContent.parent)
         {
      ...
      {
         if(this.loadingContent is LoaderInfo && LoaderInfo(this.loadingContent).loader)
         {
            try
            {
