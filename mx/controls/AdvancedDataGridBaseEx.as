package mx.controls
{
   ...
   public class AdvancedDataGridBaseEx extends AdvancedDataGridBase implements IIMESupport
   {
      ...
      override protected function adjustListContent(param1:Number = -1, param2:Number = -1) : void
      {
         ...
         if(getOptimumColumns() != visibleColumns)
         {
            _loc3_ = this.displayableColumns.length;
            _loc4_ = 0;
            _loc5_ = lockedColumnCount;
            while(_loc5_ < _loc3_)
            {
               _loc4_ = _loc4_ + this.displayableColumns[_loc5_].width;
               _loc5_++;
            }
            _loc4_ = _loc4_ + (this.getLastColumnResidualWidth() + 50);
            listSubContent.setActualSize(_loc4_,param2);
         }
         this.updateSubContent();
      }
      ...
      {
         var _loc4_:int = 0;
         var _loc1_:int = this.displayableColumns.length - 1;
         ...
         if(_loc5_ > 0 && _loc5_ < visibleColumns.length)
         {
            ...
            while(_loc4_ < _loc5_)
            {
               _loc2_ = _loc2_ - this.displayableColumns[_loc4_].width;
               _loc4_++;
            }
         }
         if(_loc1_ >= 0)
         {
            _loc3_ = !!isNaN(this.displayableColumns[_loc1_].explicitWidth)?Number(this.displayableColumns[_loc1_].preferredWidth):Number(this.displayableColumns[_loc1_].explicitWidth);
         }
         _loc4_ = _loc1_ - 1;
         while(_loc4_ >= _loc5_)
         {
            if(_loc3_ + this.displayableColumns[_loc4_].width <= _loc2_)
            {
               _loc3_ = _loc3_ + this.displayableColumns[_loc4_].width;
               _loc4_--;
               continue;
            }
            break;
      ...
      override protected function configureScrollBars() : void
      {
         ...
         if(_loc3_ + this.getHeaderItemsLength() == 0)
         {
            if(_loc1_ || _loc2_)
            {
      ...
      override protected function drawRowBackgrounds() : void
      {
         ...
         while(_loc4_ < lockedRowCount && _loc4_ < _loc8_)
         {
            this.drawRowBackground(_loc1_,_loc5_++,rowInfo[_loc4_].y,rowInfo[_loc4_].height,_loc2_[_loc7_ % _loc2_.length],_loc7_);
            _loc4_++;
            _loc7_++;
            _loc6_++;
         }
         while(_loc4_ < _loc8_)
         {
            this.drawRowBackground(_loc1_,_loc5_++,rowInfo[_loc4_].y,rowInfo[_loc4_].height,_loc2_[_loc6_ % _loc2_.length],_loc6_);
            _loc4_++;
            _loc6_++;
         }
         ...
         if(_loc3_ > lockedColumnCount)
         {
            ...
            else
            {
               while(_loc3_ > _loc10_ || _loc3_ == _loc10_ && _loc3_ > horizontalScrollPosition + lockedColumnCount && _loc12_)
               {
                  ...
                  _loc12_ = _loc11_.headerItem && _loc11_.headerItem.x + _loc11_.headerItem.width > listContent.width?1:0;
               }
            }
      ...
      private function findNextItemRenderer(param1:Boolean) : Boolean
      {
         ...
         while(!_loc4_)
         {
            ...
            if(_loc3_ >= _columns.length || _loc3_ < 0)
            {
               ...
               {
                  if(this.endEdit(AdvancedDataGridEventReason.NEW_ROW))
                  {
                     this.setEditedItemPosition(null);
                     this.deferFocus();
                     return false;
      ...
      protected function addSortField(param1:String, param2:int, param3:ICollectionView) : void
      {
         ...
         var _loc8_:ISortField = new SortField(param1);
      ...
      private function findHeaderRenderer(param1:Point) : IListItemRenderer
      {
         ...
         _loc4_ = 0;
         _loc5_ = 0;
         _loc6_ = headerItems.length;
         _loc7_ = getOptimumColumns();
         _loc8_ = 0;
         while(_loc8_ < _loc6_)
         {
         ...
         if(itemsNeedMeasurement)
         {
            ...
            if(isNaN(explicitRowHeight))
            {
               if(iterator && this.columns.length > 0)
               {
                  ...
                  columnsInvalid = true;
                  _loc2_ = getStyle("paddingTop");
                  _loc3_ = getStyle("paddingBottom");
                  _loc4_ = iterator.current;
                  _loc7_ = 0;
                  _loc8_ = this.columns.length;
                  _loc9_ = 0;
                  while(_loc9_ < _loc8_)
                  {
                     _loc6_ = this.columns[_loc9_];
                     if(_loc6_.visible)
                     {
                        _loc5_ = this.getMeasuringRenderer(_loc6_,false,_loc4_);
                        this.setupRendererFromData(_loc6_,_loc5_,_loc4_);
                        _loc7_ = Math.max(_loc7_,_loc5_.getExplicitOrMeasuredHeight() + _loc3_ + _loc2_);
                     }
                     _loc9_++;
                  }
                  setRowHeight(Math.max(_loc7_,20));
               }
               else
               {
                  setRowHeight(20);
      ...
      override protected function mouseUpHandler(param1:MouseEvent) : void
      {
         ...
         if(_loc3_ && _loc3_.data && _loc3_ != this.itemEditorInstance && this.lastItemDown == _loc3_ && _loc3_.visible && this.isDataEditable(_loc3_.data))
         {
            ...
            if(_loc7_ && _loc7_.y >= 0 && !this.dontEdit)
            {
