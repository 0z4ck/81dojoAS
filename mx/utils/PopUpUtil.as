package mx.utils
{
   ...
   public class PopUpUtil
   {
      ...
      {
         var _loc9_:Stage = null;
         var _loc6_:Matrix = MatrixUtil.getConcatenatedMatrix(param1,param3.getSandboxRoot());
         try
         {
            _loc9_ = param3.stage;
            if(_loc9_ && _loc9_.displayState != StageDisplayState.NORMAL && _loc9_.fullScreenSourceRect)
            {
               param5.x = param5.x + _loc9_.fullScreenSourceRect.x;
               param5.y = param5.y + _loc9_.fullScreenSourceRect.y;
            }
         }
         catch(e:Error)
         {
         ...
         var _loc7_:Number = DisplayObject(param4).alpha;
         var _loc8_:ColorTransform = param2;
         if(_loc8_ != null)
         {
