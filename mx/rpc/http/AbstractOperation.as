package mx.rpc.http
{
   ...
   public class AbstractOperation extends mx.rpc.AbstractOperation
   {
      ...
      public function set resultFormat(param1:String) : void
      {
         ...
         switch(param1)
         {
            ...
            case RESULT_FORMAT_FLASHVARS:
               break;
            default:
               if(param1 != null && (_loc2_ = SerializationFilter.filterForResultFormatTable[param1]) == null)
               {
                  _loc3_ = this.resourceManager.getString("rpc","invalidResultFormat",[param1,RESULT_FORMAT_OBJECT,RESULT_FORMAT_ARRAY,RESULT_FORMAT_XML,RESULT_FORMAT_E4X,RESULT_FORMAT_TEXT,RESULT_FORMAT_FLASHVARS]);
      ...
      public function sendBody(param1:Object) : AsyncToken
      {
         ...
         var _loc2_:SerializationFilter = this.getSerializationFilter();
         if(Concurrency.SINGLE == this.concurrency && activeCalls.hasActiveCalls())
         {
            _loc4_ = new AsyncToken(null);
            _loc11_ = this.resourceManager.getString("rpc","pendingCallExists");
         ...
         if(_loc8_ == CONTENT_TYPE_XML)
         {
            ...
            else if(!(param1 is XMLNode) && !(param1 is XML))
            {
               if(this.xmlEncode != null)
               {
                  ...
                  {
                     _loc4_ = new AsyncToken(null);
                     _loc7_ = this.resourceManager.getString("rpc","xmlEncodeReturnNull");
                  ...
                  {
                     _loc4_ = new AsyncToken(null);
                     _loc7_ = this.resourceManager.getString("rpc","xmlEncodeReturnNoXMLNode");
                     _loc5_ = new Fault(ERROR_ENCODING,_loc7_);
                     _loc6_ = FaultEvent.createEvent(_loc5_,_loc4_);
                     new AsyncDispatcher(dispatchRpcEvent,[_loc6_],10);
                     return _loc4_;
         ...
         else
         {
            ...
            {
               _loc4_ = new AsyncToken(null);
               _loc7_ = this.resourceManager.getString("rpc","urlNotSpecified");
            ...
            if(!this.useProxy)
            {
               _loc20_ = this.getDirectChannelSet();
               if(_loc20_ != asyncRequest.channelSet)
               {
      ...
      override mx_internal function preHandle(param1:MessageEvent) : AsyncToken
      {
         ...
         var _loc2_:Boolean = activeCalls.wasLastCall(AsyncMessage(param1.message).correlationId);
      ...
      private function decodeParameterString(param1:String) : Object
      {
         ...
         while(_loc5_ < _loc3_.length)
         {
            ...
            if(_loc7_ != -1)
            {
               _loc8_ = _loc6_.substr(0,_loc7_);
               _loc8_ = _loc8_.split("+").join(" ");
               _loc8_ = unescape(_loc8_);
               _loc9_ = _loc6_.substr(_loc7_ + 1);
