package mx.messaging.messages
{
   public class MessagePerformanceUtils
   {
      ...
      public function prettyPrint() : String
      {
         ...
         {
            _loc1_ = _loc1_ + ("Original message size(B): " + this.messageSize + "\n");
         }
         if(this.responseMessageSize != 0)
         {
            _loc1_ = _loc1_ + ("Response message size(B): " + this.responseMessageSize + "\n");
         }
         if(this.totalTime != 0)
         {
            _loc1_ = _loc1_ + ("Total time (s): " + this.totalTime / 1000 + "\n");
         }
         if(this.networkRTT != 0)
         {
            _loc1_ = _loc1_ + ("Network Roundtrip time (s): " + this.networkRTT / 1000 + "\n");
         }
         if(this.serverProcessingTime != 0)
         {
            _loc1_ = _loc1_ + ("Server processing time (s): " + this.serverProcessingTime / 1000 + "\n");
         }
         if(this.serverAdapterTime != 0)
         {
            _loc1_ = _loc1_ + ("Server adapter time (s): " + this.serverAdapterTime / 1000 + "\n");
         }
         if(this.serverNonAdapterTime != 0)
         {
            _loc1_ = _loc1_ + ("Server non-adapter time (s): " + this.serverNonAdapterTime / 1000 + "\n");
         }
         if(this.serverAdapterExternalTime != 0)
         {
            _loc1_ = _loc1_ + ("Server adapter external time (s): " + this.serverAdapterExternalTime / 1000 + "\n");
         }
         if(this.pushedMessageFlag)
         {
            _loc1_ = _loc1_ + "PUSHED MESSAGE INFORMATION:\n";
            if(this.totalPushTime != 0)
            {
               _loc1_ = _loc1_ + ("Total push time (s): " + this.totalPushTime / 1000 + "\n");
            }
            if(this.pushOneWayTime != 0)
            {
               _loc1_ = _loc1_ + ("Push one way time (s): " + this.pushOneWayTime / 1000 + "\n");
            }
            if(this.originatingMessageSize != 0)
            {
               _loc1_ = _loc1_ + ("Originating Message size (B): " + this.originatingMessageSize + "\n");
            }
            if(this.serverPollDelay != 0)
            {
               _loc1_ = _loc1_ + ("Server poll delay (s): " + this.serverPollDelay / 1000 + "\n");
            }
         }
         return _loc1_;
      }
