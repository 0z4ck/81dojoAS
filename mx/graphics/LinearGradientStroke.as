package mx.graphics
{
   ...
   public class LinearGradientStroke extends GradientStroke
   {
      ...
      override public function apply(param1:Graphics, param2:Rectangle, param3:Point) : void
      {
         ...
         param1.lineGradientStyle(GradientType.LINEAR,colors,alphas,ratios,commonMatrix,spreadMethod,interpolationMethod);
      }
         ...
         if(!compoundTransform)
         {
            ...
            if(isNaN(_loc6_))
            {
               if(rotation % 90 != 0)
               {
                  ...
                  _loc10_ = Math.acos(param1.width / _loc9_) * 180 / Math.PI;
                  if(_loc7_ > _loc10_)
                  {
                     _loc7_ = 90 - _loc7_;
                     _loc8_ = param1.height;
                  }
                  _loc6_ = _loc8_ / Math.cos(_loc7_ / 180 * Math.PI);
               }
               else
               {
                  _loc6_ = rotation % 180 == 0?Number(param1.width):Number(param1.height);
               }
            }
            ...
            param2.rotate(!isNaN(_angle)?Number(_angle):Number(rotationInRadians));
            if(isNaN(_loc4_))
            {
